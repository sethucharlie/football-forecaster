...
/**
 * MarketAccuracyChart — Recharts BarChart.
 * Props: data = [{ market, win_rate }]
 */
export default function MarketAccuracyChart({ data }) {
  ...
  return (
    <div className="bg-[var(--color-card)] rounded-lg p-[var(--spacing-sm)] border border-[var(--color-border)]">
      <h3 className="text-white font-bold text-sm mb-3">Market Accuracy</h3>
      <div className="h-64 md:h-80">
        <ResponsiveContainer width="100%" height="100%">
          <BarChart data={chartData} margin={{ top: 5, right: 20, left: 10, bottom: 40 }}>
            ...
            />
            <Bar dataKey="winRatePercent" fill="var(--color-primary)" radius={[4, 4, 0, 0]} />
          </BarChart>
        </ResponsiveContainer>
      </div>
    </div>
